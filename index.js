                        ...
                        if(err || results.rows.length==0){
                            ...
                            client.query("select * from userhabit where type != ''", (err, results) =>{
                                if(err || results.rows.length==0){
                                    ...
                                    client.query("SELECT book.bookno ,book.bookname, book.type, book.picture FROM book, userhabit where book.type=userhabit.type AND userid= $1 AND count>100 order by count,random()", [userId], (err, results) => {    
                                        if(err || results.rows.length==0){
                                            client.query("select * from book ORDER BY RANDOM() LIMIT 3", (err, results) => {
                                                console.log(results);
